...
#define TREE_NUMBER 89                   //Quantas arvores a serem feitas (+ = +tempo)
#define SAMPLE_SIZE 150                  //Quantas samples por arvore (+ = +tempo)
#define FEATURES_PER_TREE 105            //Quantas features por arvore(+ = +tempo)
#define MIN_SAMPLES_PER_NODE 39          //Minimo da samples para um nó ser considerado uma folha(+ = -tempo)
...
#define VERBOSE 1                         //Eu peguei essa ideia do copilot, defina como 0 para diminuir os prints
//--------------------------------------------------------------------------------------------------------------------------//
    ...
    while (true){
        ...
        for (int i = 0; i < testeLinhas; i++)
        {
            ...
            for (int j = 0; j < TREE_NUMBER; j++)
            {
                ...
                if (testStructure(forest[j], testTable[i], numeroFeatures) == 1)
                {
                    positivos++;
                }

                // Anotar quantas arvores são invalidas
                else if (testStructure(forest[j], testTable[i], numeroFeatures) == -1)
                {
                    invalidas++;
                }
            }
